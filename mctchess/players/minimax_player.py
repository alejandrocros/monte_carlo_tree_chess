...
class MiniMaxPlayer(Player):
    ...
    def play(self, board: Board, debug: bool = False) -> Union[str, Tuple[float, str]]:
        if self.ab_pruning:
            best_score, best_move = self.minimax_pruned(board=board, depth=self.depth)
        else:
    ...
    def minimax(
        self, board: Board, depth: int = 2, add_mobility: bool = False
    ) -> Tuple[float, str]:
        player = 2 * int(board.turn) - 1  # 1 for white, -1 for black
        if depth == 0 or board.outcome() is not None:
            score = (
                self.evaluate(board) if not board.is_checkmate() else -player * np.inf
            )
            return score, str()
        else:
            final_eval = dict()
            ...
            for move in legal_moves:
                board.push(move)
                final_eval[str(move)] = self.minimax(
    ...
    def minimax_pruned(self, board: Board, depth: int = 2) -> Tuple[float, str]:
